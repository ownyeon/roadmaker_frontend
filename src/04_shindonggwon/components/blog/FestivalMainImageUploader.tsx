...
interface FestivalMainImageUploaderProps {
  setMainImage: React.Dispatch<React.SetStateAction<string | null>>;
}

const FestivalMainImageUploader = ({ setMainImage }: FestivalMainImageUploaderProps) => {
  const [mainImages, setMainImages] = useState<string[]>([]);  // 업로드된 이미지를 상태로 관리
        ...
        img.onload = () => {
          ...
          if (img.width > maxSize || img.height > maxSize) {
            ...
            if (newImages.length === fileList.length) {
              setMainImages((prev) => [...prev, ...newImages]);  // 업로드된 이미지 배열을 상태에 추가
              setMainImage(newImages[0]);  // 첫 번째 이미지를 부모 컴포넌트로 전달
              setError("");  // 에러 초기화
            }
          }
        };
  ...
  return (
    <div className="row x-gap-20 y-gap-20 pt-15">
      ...
      <div className="col-auto">
        <div className="w-200">
          <label htmlFor="bannerUpload" className="d-flex ratio ratio-1:1 border rounded-4">
            <div className="flex-center flex-column text-center bg-blue-2 h-full w-1/1 absolute rounded-4 border-type-1">
              ...
              <div className="text-blue-1 fw-500">Upload Main Images</div>
            </div>
          </label>
          <input
            type="file"
...
export default FestivalMainImageUploader;
