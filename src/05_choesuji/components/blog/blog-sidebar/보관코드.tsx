...
const regionThemes1 = Object.fromEntries(
  regions.map((region) => [region, themes.slice(1)]) // 첫 번째 전체 버튼 제외하고 테마 설정
);
...
//주요도시 (시도)
 const handleRegionClick = (region: string) => {
  ...
  if (region === "#전국") {
    ...
    // 해당 지역에 해당하는 하위 지역을 추가
    const subRegionForSelected = subRegions[region] || [];
    updatedKeywords = [...updatedKeywords, ...subRegionForSelected];

    setSelectedKeywords(updatedKeywords); // 주요지역과 하위지역을 모두 적용
  }
};
